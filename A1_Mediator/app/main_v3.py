...
@app.get("/a1-p/policytypes")
async def get_all_policy_types():
    try:
        conn = pymysql.connect(**DB_CONFIG)
        with conn.cursor() as cursor:
            cursor.execute("""
                SELECT DISTINCT policy_type_id, app_name 
                FROM xapp_policies 
                WHERE is_active = 1
            """)
            rows = cursor.fetchall()
            return [{"policy_type_id": row[0], "app_name": row[1]} for row in rows]
    ...
    try:
        ...
        with conn.cursor() as cursor:
            cursor.execute("""
                SELECT app_name 
                FROM xapp_policies 
                WHERE policy_type_id = %s AND is_active = 1
            """, (policy_type_id,))
            row = cursor.fetchone()
            if not row:
            ...
            # ✅ 若 xApp 啟動成功（200 ~ 299），則更新 is_running
            if 200 <= response.status_code < 300:
                cursor.execute("""
                    UPDATE xapp_policies 
                    SET is_running = 1 
                    WHERE policy_type_id = %s AND app_name = %s
                """, (policy_type_id, app_name))
                conn.commit()
